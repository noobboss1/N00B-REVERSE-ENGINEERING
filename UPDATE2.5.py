...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/SHIN-DATA/DUMP')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
		back()
	if len(vin)==0:
		haha = '# ANDA TIDAK MEMILIKI HASIL FILE DUMP'
		sol().print(mark(haha, style='yellow'))
		time.sleep(2)
		back()
	else:
		gerr = '# HASIL DUMP FILE ANDA'
		sol().print(mark(gerr, style='bold green'))
		cih = 0
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/SHIN-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
				...
				lol.update({str(cih):str(isi)})
				print('['+str(cih)+'] '+isi+' [ '+str(len(hem))+' Account ]'+x)
		gerr2 = '# PILIH HASIL UNTUK MULAI CRACK'
		sol().print(mark(gerr2, style='green'))
		geeh = input(x+'['+p+'Shinzy'+x+'] choose : ')
		try:geh = lol[geeh]
		except KeyError:
			ric = '# OPSI TIDAK ADA DI MENU'
			sol().print(mark(ric, style='red'))
			exit()
		...
		setting()
def tipsx():
	print('NEXT UPDATE KAKAK, HIDUP SAYA BUKAN HANYA KODING DOANG, ADA ORANG YANG INGIN SAYA BAHAGIAKAN JUGA')
